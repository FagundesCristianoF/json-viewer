...
mod tests {
    ...
    #[test]
    fn filter_bool_literal() {
        let a = arena(r#"[{"active": true}, {"active": false}]"#);
        let p = paths(&a, "$[?(@.active == true)]");
        assert_eq!(p, vec!["$[0]"]);
    }

    #[test]
    fn filter_null_literal() {
        let a = arena(r#"[{"v": null}, {"v": 1}]"#);
        let p = paths(&a, "$[?(@.v == null)]");
        assert_eq!(p, vec!["$[0]"]);
    }

    #[test]
    fn filter_ne_operator() {
        let a = arena(r#"[{"t": "x"}, {"t": "y"}, {"t": "z"}]"#);
        let mut p = paths(&a, "$[?(@.t != 'x')]");
        p.sort();
        assert_eq!(p, vec!["$[1]", "$[2]"]);
    }

    #[test]
    fn filter_le_ge_operators() {
        let a = arena(r#"{"vals": [{"n": 1}, {"n": 5}, {"n": 10}]}"#);
        let le = paths(&a, "$.vals[?(@.n <= 5)]");
        assert_eq!(le.len(), 2);
        let ge = paths(&a, "$.vals[?(@.n >= 5)]");
        assert_eq!(ge.len(), 2);
    }

    #[test]
    fn descendant_wildcard() {
        let a = arena(r#"{"a": {"b": {"c": 1}}}"#);
        let p = paths(&a, "$..*");
        // visits every node below root
        assert!(p.len() >= 3);
        assert!(p.contains(&"$.a".to_string()));
        assert!(p.contains(&"$.a.b".to_string()));
        assert!(p.contains(&"$.a.b.c".to_string()));
    }

    #[test]
    fn double_quoted_bracket_key() {
        let a = arena(r#"{"hello world": 5}"#);
        assert_eq!(paths(&a, r#"$["hello world"]"#), vec!["$['hello world']"]);
    }

    #[test]
    fn index_out_of_bounds_empty() {
        let a = arena(r#"[1, 2]"#);
        assert!(query(&a, "$.a[99]").unwrap().is_empty());
    }

    #[test]
    fn deep_recursive_descent() {
        let a = arena(r#"{"a": {"id": 1}, "b": [{"id": 2}, {"c": {"id": 3}}]}"#);
        let mut p = paths(&a, "$..id");
        p.sort();
        assert_eq!(p.len(), 3);
    }
    ...
    fn root_only_expr() {
        let a = arena(r#"{"x": 1}"#);
        let hits = query(&a, "$").unwrap();
        assert_eq!(hits, vec![a.root]);
    }
}
